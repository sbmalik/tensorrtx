 ...
 *     0 for success, else non-zero (which will cause engine termination). 
 */
int CustomPlugin::enqueue(int batchSize, const void *const *inputs, void **outputs, void *workspace,
                          cudaStream_t stream) {
    // 0 for success, else non-zero (which will cause engine termination).
 ...
 */
IPluginV2 *
CustomPluginCreator::deserializePlugin(const char *name, const void *serialData, size_t serialLength) {
    return new CustomPlugin(name);
}
