 ...
 *     0 for success, else non-zero (which will cause engine termination). 
 */
int CustomPlugin::enqueue(int batchSize, const void *const *inputs, void *const *outputs, void *workspace,
 ...
 *     The workspace size.
 */
size_t CustomPlugin::getWorkspaceSize(int batchSize) const noexcept {
    return 0;
}
 ...
 */
void
CustomPlugin::configureWithFormat(const Dims *inputs, int nbInputs, const Dims *outputs, int nbOutputs, DataType type,
 ...
 *     PluginV2IOExt or PluginV2DynamicExt for other PluginFormats.
 */
bool CustomPlugin::supportsFormat(DataType type, PluginFormat format) const noexcept {
    return true;
}
 ...
 */
void CustomPlugin::destroy() noexcept { delete this; }
 ...
 *     A Plugin object.
 */
IPluginV2 *CustomPlugin::clone() const noexcept {
 ...
 */
void CustomPlugin::setPluginNamespace(const char *libNamespace) noexcept {
 ...
 */
IPluginV2 *
CustomPluginCreator::deserializePlugin(const char *name, const void *serialData, size_t serialLength) noexcept {
    return new CustomPlugin(name);
}
 ...
 */
void CustomPluginCreator::setPluginNamespace(const char *pluginNamespace) noexcept {
 ...
 *     The namespace of the plugin object.
 */
const char *CustomPluginCreator::getPluginNamespace() const noexcept {
    return mNamespace.c_str();
}
